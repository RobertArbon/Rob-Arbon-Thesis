...
\begin{table}
    \mycaption{The hyper-parameter search space of alanine dipeptide. Randomly sampled values of $\mathbf{x} = (\chi, n)$ were used to fit and score a MSM to create the hyper-parameter trial data set. The Cartesian coordinates were first aligned to a single, randomly chosen, trajectory frame so that features (2) and (5) did not include spurious rotational or translational motion.}
    \centering
    ...
    \textbf{Hyper-parameter} & \textbf{Type} & \textbf{Search space} & \textbf{Details} \\
     \hline\hline
    Feature, $\chi$ & Categorical & (1) $(\phi, \psi)$ torsions &   \\
    & & (2) (x, y, z) coordinates & Heavy atoms only  \\
    & & (3) $\phi$ torsion &  \\ 
    & & (4) $\psi$ torsion &  \\ 
...
\begin{table}
    \mycaption{The hyperparameter search space for AADH. Randomly sampled values of $\mathbf{x} = (\chi, \tau, m, n)$ were used to fit and score a MSM to create the hyper-parameter trial data set. TICA was applied to every feature except RMSD. Torsional angles, $\theta$, were given $(\sin(\theta),\cos(\theta)$ representations. The Cartesian coordinates were first aligned to a single, randomly chosen, trajectory frame so that feature (5) did not include spurious rotational or translational motion.}
    \centering
...
A GP is specified by a mean function, $\mu(\mathbf{x})$ and a covariance function, $k(\mathbf{x}, \mathbf{x}^{\prime})$ (this section will follow the exposition and conventions in \cite{rasmussenGaussianProcessesMachine2006}) where $\mathbf{x}$ are some arbitrary inputs. The GP, $f$, is related to these functions as follows: 
