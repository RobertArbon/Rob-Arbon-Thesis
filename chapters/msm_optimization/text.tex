...
The VAMP-2 score is a suitable measurement of the quality of the model hyperparameters and can be compared between different models. Each set of hyperparameter defines a different set of basis states which are used to represent the same underlying MD data. The eigenvectors of the transition matrix are the optimal set of reaction coordinates for representing the slow dynamics present in the MD data \cite{nuskeVariationalApproachMolecular2014}. VAMP-2 measures how well a restricted number of eigenvectors, the slow eigenvectors, reproduce the dynamics. It can be thought as minimizing two different types of error (although they are related). The first is the error arising from a description of the dynamics incorporating only the slow dynamical processes. If the error is small then the slow eigenfunctions, whose accuracy is determined by the basis states, provide an satisfactory description. Maximizing the VAMP-2 score minimizes this error \cite{wuVariationalApproachLearning2020c}. The second can be seen by defining a set of new coordinates based on eigenvectors of the transition matrix. Distances in this new space, the kinetic distance,  define how slowly two conformations interconvert \cite{noeKineticDistanceKinetic2015}. Using only the slow eigenvectors will reduce the accuracy of this kinetic distance (compared to using all of the eigenvectors), maximizing the VAMP-2 score minimizes this error by maximizing the kinetic distance of the reduced dimensionality description. 
