...
\KwData{scoring function: $CV(\theta; \mathcal{D})$}
\Par{number of iterations: $J$}
\KwResult{Cross-validated score: $\operatorname{MCV}(\theta)$}
\BlankLine
\For{$i\leftarrow 1$ \KwTo $J$}{
    split trajectories, $\mathcal{D}$, into two disjoint sets, $\mathcal{D}^{\mathrm{train}}$ and  $\mathcal{D}^{\mathrm{test}}$
    estimate model at using $\mathcal{D}^{\mathrm{train}}$ and $\theta$
