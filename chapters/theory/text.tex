...
Kinetic mapping is recommended for TICA dimensionality reduction \cite{noeKineticDistanceKinetic2015}, however a recent study which looked at the quality of MSM models created with and without kinetic mapping  \cite{husicOptimizedParameterSelection2016} for a number of different 


however, its utility in improving model performance is data set dependent. Both k-means and Ward clustering have been shown to give good performance in MSMs \cite{husicWardClusteringImproves2017a}. Fixing both of these hyperparameters reflects an arbitrary decision to keep the number of hyperparameters to a minimum.
