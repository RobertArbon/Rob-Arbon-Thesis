...
\cite{schutteDirectApproachConformational1999} Quantitatively and qualitatively describing the slow conformational dynamics of biomolecular systems is of central importance for understanding their function, chemical, physical and biological properties. For example, conformational changes are essential to the catalytic process of enzymes [], how drugs and other small molecules bind to proteins regulating their behaviour [], or how cells pump ions [] or send signals between themselves []. 

Computational approaches to studying conformational dynamics are important because their allow descriptions with high temporal and spatial resolution [] beyond the scope of most experimental techniques []. Typically, one evolves the systems equations of motion using molecular dynamics (MD) to produce a set of trajectories through phase space. These trajectories can be analysed to reveal conformational transition pathways and metastable states [] and other properties of interest. 

There are a wide range of techniques for understanding conformational dynamics from MD simulations which depend on current knowledge of the system and the questions being asked. Path based techniques such as transition path sampling (TPS) \cite{bolhuisTRANSITIONPATHSAMPLING2002, dellagoTransitionPathSampling1999, dellagoTransitionPathSampling2002a}, transition interface sampling (TIS) \cite{} and forward flux sampling (FFS) 
\cite{} all start with two specified stable conformations, A & B, and can be used to estimate rate constants and reaction coordinates without previous knowledge of transition states. TPS works by first proposing an initial series of discrete states between A and B known as as transition pathway. Then a statistical ensemble of pathways is generated from this initial path using Metropolis Monte Carlo. TIS and FFS are similar but define interfaces based on an order parameter which separate A and B. These techniques have been applied \cite{} to the model protein folding system Trp-cage \cite{neidighDesigning20residueProtein2002} to elucidate the folding pathway and have replicated experimental values for the folding and unfolding rates.  

TPS was used to elucidate  \cite{juraszekSamplingMultipleFolding2006} the folding mechanism of mini-protein Trp-cage . Variations of TPS, such as transition interface sampling (TIS) and forward flux sampling (FFS) have also been applied to the Trp-cage system.  Both techniques break up the pathway from A to B using interfaces   is a fast folding protein which predominantly transitions from its native (N) to unfolded state (U) via a loop intermediate (L).  Given knowledge of these states and their structures several 'path' based methods have been used to estimate rate constants and free energies as well as elucidate important conformations along the folding pathway.  FFS \cite{velez-vegaKineticsMechanismUnfolding2010}

Transition path sampling  and associated techniques adapted for biomolecular systems (partial path transition interface sampling (PPTIS) ) are suited to characterising reaction mechanisms and rates, given two metastable states. They 
% \cite{bolhuisTRANSITIONPATHSAMPLING2002,dellagoTransitionPathSampling2002}
% \cite{moroniRateConstantsDiffusive2004}
ZTS, FTS, MSMs, Milestoning, PCA (Kernel PCA), (MDS), Isomap, Sketch-map, Diffusion-map, locally scaled diffusion map. 
